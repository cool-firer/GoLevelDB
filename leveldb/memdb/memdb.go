...
func (i *dbIter) fill(checkStart, checkLimit bool) bool {
	if i.node != 0 {
		n := i.p.nodeData[i.node] // kvoffset
		m := n + i.p.nodeData[i.node+nKey] // kvoffset + len(key)
		i.key = i.p.kvData[n:m] // 这里就是取到了 key
		if i.slice != nil {
			switch {
...
func (i *dbIter) First() bool {
	...
	if i.slice != nil && i.slice.Start != nil {
		...
		i.node = i.p.nodeData[nNext] // head结点 第0层的下一个结点 也就是第一个结点
	}
	return i.fill(false, true)
}
...
func (i *dbIter) Next() bool {
	...
	if i.node == 0 { // &dbIter{p: p, slice: nil}
		if !i.forward {
			return i.First()
		}
		return false
...
func (p *DB) findGE(key []byte, prev bool) (int, bool) {
	...
	h := p.maxHeight - 1 // h: 0
	for {
		next := p.nodeData[node+nNext+h] // nNext: 4, next: 0
		cmp := 1
		if next != 0 {
			o := p.nodeData[next]
			// 表里的key > key 1; 
		...
		if cmp < 0 {
			...
			if prev {
				p.prevNode[h] = node // 与书里的last语义是一样的, 
			} else if cmp == 0 {
				return next, true
			}
			if h == 0 {
...
func (p *DB) Put(key []byte, value []byte) error {
	...
	kvOffset := len(p.kvData) // 0
	p.kvData = append(p.kvData, key...)
	p.kvData = append(p.kvData, value...) // key 8字节(seq << 8 | kt) value
	// Node
	node := len(p.nodeData) // 16

	p.nodeData = append(p.nodeData, kvOffset, len(key), len(value), h)
	for i, n := range p.prevNode[:h] {
		m := n + nNext + i // 4 n当前节点 + 固定偏移4 + 当前遍历的层级 = 下一节点下标
		p.nodeData = append(p.nodeData, p.nodeData[m])
		p.nodeData[m] = node // 指向当前node
	}
...
func (p *DB) Free() int {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return cap(p.kvData) - len(p.kvData) // 可用的
}
