...
func OpenFile(path string, readOnly bool) (Storage, error) {
	// path: "/Users/luke/Desktop/work/go-ethereum1.10/XXX_DATA/chain_data/geth/chaindata"
...
func (fs *fileStorage) Lock() (Locker, error) {
	...
	fs.slock = &fileStorageLock{fs: fs} // 又TM的指回自己
	return fs.slock, nil
}
...
func (fs *fileStorage) doLog(t time.Time, str string) {
	if fs.logSize > logSizeThreshold { // 1024 * 1024 // 1 MiB
		// Rotate log file.
		fs.logw.Close()
		fs.logw = nil
		fs.logSize = 0
	...
	fs.printDay(t)
	hour, min, sec := t.Clock() // 假如现在 14点36分23秒45毫秒
	msec := t.Nanosecond() / 1e3
	// time
	fs.buf = itoa(fs.buf[:0], hour, 2) // buf: [ '1', '4', ]
	...
	fs.buf = append(fs.buf, ' ') // "14:36:23:000045 "
...
func (fs *fileStorage) setMeta(fd FileDesc) error {
	...
	// chain_data/geth/chaindata/CURRENT.0
...
func (fs *fileStorage) List(ft FileType) (fds []FileDesc, err error) {
	...
	dir, err := os.Open(fs.path) // fs.path: "XXX_DATA/chain_data/geth/chaindata"
	if err != nil {
		return
	}
