...
func (db *DB) memCompaction() {
	...
	// 刚推入的0层sstable, 信息都在rec里面, 怎么传给 tcom ??
...
func (b *tableCompactionBuilder) run(cnt *compactionTransactCounter) error {
	...
	// 一整排 遍历每一个sstable, 取最小的放在keys
	// 取完第一个后, i.index在最小key位置, dir改为dirForward
	for i := 0; iter.Next(); i++ {
...
func (db *DB) tableCompaction(c *compaction, noTrivial bool) {
	...
	rec := &sessionRecord{}
	rec.addCompPtr(c.sourceLevel, c.imax) // 记录最大key

	if !noTrivial && c.trivial() {
		t := c.levels[0][0] // 直接把t加到 sourceLevel + 1
	...
	for i, tables := range c.levels {
		for _, t := range tables {
			...
			rec.delTable(c.sourceLevel+i, t.fd.Num) // 从相应层中删除
		}
	}
