...
func newVersion(s *session) *version {
	id := atomic.AddInt64(&s.ntVersionId, 1) // 增加返回新值
...
func (v *version) computeCompaction() {
	...
	for level, tables := range v.levels {
		...
		if score > bestScore { // 最需要comppact的 level
			bestLevel = level
			bestScore = score
		}
...
type versionStaging struct {
	base   *version
	levels []tablesScratch // 0 - 7级的文件改变
}
...
func (p *versionStaging) finish(trivial bool) *version {
	// Build new version. 空的version
	nv := newVersion(p.base.s)
	numLevel := len(p.levels)
	if len(p.base.levels) > numLevel { // 之前版本的level大
		numLevel = len(p.base.levels) // 取大的
	}
	nv.levels = make([]tFiles, numLevel)
	for level := 0; level < numLevel; level++ {
		var baseTabels tFiles //  原版本这层的文件
		if level < len(p.base.levels) {
			baseTabels = p.base.levels[level]
		}

		if level < len(p.levels) {
			scratch := p.levels[level] // 现版本这层的文件
			...
			var nt tFiles
			// Prealloc list if possible. 与上个版本的增量 增加了多少 减少了多少
