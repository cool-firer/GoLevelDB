...
func (g *bloomFilterGenerator) Generate(b Buffer) {
	...
	nBytes := (nBits + 7) / 8 // 这么多位数需要的字节
	nBits = nBytes * 8 // 位数取整

	dest := b.Alloc(int(nBytes) + 1) // 分配足够的字节
	dest[nBytes] = g.k // k: 6 最后一个字节存hash函数个数
	for _, kh := range g.keyHashes {
		delta := (kh >> 17) | (kh << 15) // Rotate right 17 bits

		for j := uint8(0); j < g.k; j++ {
			bitpos := kh % nBits // hash值 % 位数
			dest[bitpos/8] |= (1 << (bitpos % 8)) // 该位设为1
			kh += delta // 每次运算的kh都会不同
		}
	}
