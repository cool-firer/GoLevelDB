...
func (tf tFiles) getOverlaps(dst tFiles, icmp *iComparer, umin, umax []byte, overlapped bool) tFiles {
	...
	for i := 0; i < len(tf); {
		...
		if t.overlaps(icmp, umin, umax) {
			...
			// dst找到的是这一层里, 与min, max有交叉的sstable, 包括min max
			dst = append(dst, t)
		}
		i++
	}
...
func (t *tOps) createFrom(src iterator.Iterator) (f *tFile, n int, err error) {
	...
	for src.Next() {
		// src.Key()遍历的key, value
		// 存入w.dataBlock.buf 超过4kib就生成一个block写入文件 000001.ldb
		// 记录一个entry在index block
...
func (t *tOps) open(f *tFile) (ch *cache.Handle, err error) {
	ch = t.cache.Get(0, uint64(f.fd.Num), func() (size int, value cache.Value) {
		...
		r, err = t.s.stor.Open(f.fd) // 打开文件, 返回一个包裹的iStorageReader
		if err != nil {
			return 0, nil
		}
...
func newTableOps(s *session) *tOps {
	...
	if !s.o.GetDisableBlockCache() {
		...
		if s.o.GetBlockCacheCapacity() > 0 { // 8MiB
