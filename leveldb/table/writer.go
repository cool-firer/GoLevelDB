...
func (w *filterWriter) flush(offset uint64) { // 0
	if w.generator == nil {
		return
	}
	// baseLg: 11 已经写入的量 > 2^11
...
func (w *filterWriter) generate() {
	...
	// Generate filters.
	if w.nKeys > 0 {
		w.generator.Generate(&w.buf) // 填充buf
		w.nKeys = 0
	}
}
...
func (w *Writer) finishBlock() error {
	w.dataBlock.finish() // 在data entry后面加restart
	...
	w.filterBlock.flush(w.offset) // 开始算hash, 但还没写文件
	return nil
}
...
func (w *Writer) Append(key, value []byte) error {
	...
	w.flushPendingBH(key) // 每生成一个data block, 就有一个entrie在index block
...
func (w *Writer) Close() error {
	...
	w.flushPendingBH(nil) // index block

	// Write the filter block.
	var filterBH blockHandle

	// 拼接 filter data && offset
	...
	if filterBH.length > 0 {
		// filter.leveldb.BuiltinBloomFilter
		key := []byte("filter." + w.filter.Name())
		n := encodeBlockHandle(w.scratch[:20], filterBH) // 往filterBH写filterblock的offset length
	...
	footer := w.scratch[:footerLen] // 48
	for i := range footer {
		footer[i] = 0
	}
	n := encodeBlockHandle(footer, metaindexBH) // metaindex的offset length
	encodeBlockHandle(footer[n:], indexBH) // index
