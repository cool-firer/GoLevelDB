...
func (b *Batch) grow(n int) {
	o := len(b.data)
	if cap(b.data)-o < n { // b.data剩余空间 < n
		div := 1
		if len(b.index) > batchGrowRec { // batchGrowRec: 3000
...
func (b *Batch) appendRec(kt keyType, key, value []byte) {
	n := 1 + binary.MaxVarintLen32 + len(key) // 1 + 5 + len("key")
	if kt == keyTypeVal {
		n += binary.MaxVarintLen32 + len(value) // 1+5+len("key") + 5+len("value")
	}
	b.grow(n) // 扩大b.data, 以便存下原有的data和n
